...
### 新功能界面 Feature UIs
点亮后端已有、前端缺失的能力：
- **健康记录附件**：记录卡片新增附件区——图片缩略图 / 文件链接、上传（多选 图片/PDF）与删除。新增 `resolveUpload()` 将 `/uploads/...` 解析到后端源；后端为 `/uploads` 放宽 Helmet 的 `Cross-Origin-Resource-Policy` 为 `cross-origin`，使前端可跨源加载附件（其余路由仍严格）。
- **体重趋势追踪**：健康页新增「体重趋势」卡片——当前体重 + 增减徽章、SVG 折线趋势图（渐变填充、数据点标注）、历史记录列表与删除，配「记录体重」弹窗。
- **修改密码**：个人中心新增「修改密码」卡片（当前/新/确认密码），改后吊销其它会话。
- **注销账号**：危险区按钮接通后端软删除 + 自动登出。
- **提醒标记完成**：提醒卡片新增 ✓ 按钮，完成后从列表移除（保留历史，不再只能删除）。
- **点赞/取消点赞**：社区点赞改为可切换的乐观更新（高亮态 + 计数增减）。
- `services.jsx` 补齐对应 api 与 demo 实现：`weight`、`users.changePassword/delete`、`reminders.complete`、`community.unlike`。
